...
/***/ (function(module, exports, __webpack_require__) {
	...
	__weex_define__('@weex-component/886e5ef2010cf873d3e944f3ea253b6f', [], function(__weex_require__, __weex_exports__, __weex_module__) {
	...
	__weex_bootstrap__('@weex-component/886e5ef2010cf873d3e944f3ea253b6f',undefined,undefined)
	          ...
	          "children": [
	            {
	              ...
	              "children": [
	                ...
	                },
	                {
	                  ...
	                  "attr": {
	                    "src": function () {return this.setDefault?'http://app5.bac365.com:10080/web_image/iconImage/on.png':'http://app5.bac365.com:10080/web_image/iconImage/off.png'}
	                  },
	                  "style": {
	                    "height": "50wx",
	                    "width": "60wx"
...
/***/ (function(module, exports, __webpack_require__) {
	...
	module.exports = {
	    ...
	    methods: {
	        ...
	        todetailAddress: function todetailAddress() {

	            if (this.lastAddrss.address) {} else {
	                ...
	                storage.setItem('addressName', this.name, function (e) {});
	                storage.setItem('addressPhone', this.userInfo.currentPhone, function (e) {});

	                var vm = this;

	                var params = {
	                    'url': vm.baseURL + 'address/detailsAddress.js',
	                    'animated': 'true'
	                };
	        ...
	        onchangePhone: function onchangePhone(event) {
	        ...
	        toAddress: function toAddress() {
	            ...
	            if (this.comeFrom === "me") {
	                params = {
	                    'url': this.baseURL + 'address/detailsAddress.js?from=me&address_id=' + this.lastAddrss.address_id,
	                    'animated': 'true'
	                };
	            } else {
	                params = {
	                    'url': this.baseURL + 'address/detailsAddress.js?address_id=' + this.lastAddrss.address_id,
	                    'animated': 'true'
	                };
	            }
	            storage.setItem('addressName', this.name, function (e) {});
	            storage.setItem('addressPhone', this.phone, function (e) {});
	        ...
	        getHistoryAddress: function getHistoryAddress() {
	            var self = this;
	            var listMap = [];
	            if (self.address_id != "null" && self.address_id.length > 0) {
	                listMap = [{
	            ...
	            modal.httpServer({ "methodName": "ACTIVITIE.QUERY_MAILING_ADDRESS",
	                ...
	                if (ret.data.listMap.length > 0) {
	                    ...
	                    self.address = self.lastAddrss.address;
	                    self.name = self.lastAddrss.recipient_name;
	                    self.phone = self.lastAddrss.recipient_phone;
	                    self.address_id = self.lastAddrss.address_id;
	                    self.setDefault = self.lastAddrss.is_default;
	                }
	            });
	        },
	        ...
	        btnAction: function btnAction() {
	            ...
	            if (self.address.length == 0) {
	                self.toast('请填写详细的邮寄地址');
	                return;
	            }
	            var modal = __weex_require__('@weex-module/httpModule');
	            if (self.lastAddrss.address_id) {
	                modal.httpServer({ "methodName": "ACTIVITIE.UPDATE_MAILING_ADDRESS",
	                    "actionType": 0,
	                    "listMap": [{ "address_id": self.lastAddrss.address_id,
	                        ...
	                        "is_default": self.setDefault ? 1 : 0 }]
	                }, function (ret) {
	                    if (ret.data.errorId == 0) {

	                        var params = {
	                            'url': self.baseURL + 'address/addressList.js?from=' + self.comeFrom,
	                            'animated': 'true'
	                        };
	                ...
	                modal.httpServer({ "methodName": "ACTIVITIE.ADD_MAILING_ADDRESS",
	                    ...
	                    if (ret.data.errorId == 0) {
	                        var params = {
	                            'url': self.baseURL + 'address/addressList.js',
	                            'animated': 'true'
	                        };
	                        self.$call('navigator', 'push', params, function () {});
	                    }
	        ...
	        APPLY_ACTIVITY: function APPLY_ACTIVITY() {
	            var modal = __weex_require__('@weex-module/httpModule');
	            modal.httpServer({ "methodName": "APPLY_ACTIVITY",
	                "actionType": 0,
	                "listMap": [{
	                    "customers_id": self.userInfo.customers_id,
	                    "login_phone": self.userInfo.currentPhone,
	                    "adv_activity_id": self.adv_activity_id,
	                    "car_id": self.carInfo.car_id,
	                    "car_license_no": self.carInfo.car_license_no,
	                    "address_id": self.lastAddrss.address_id

	                }]
	            }, function (ret) {
	                if (ret.data.errorId == 0) {
	                    var params = {
	                        'url': self.baseURL + 'activity/activityResult.js',
	                        'animated': 'true'
	                    };
	                    self.$call('navigator', 'push', params, function () {});
	                }
	    ...
	    created: function created() {
	        ...
	        self.comeFrom = decodeURIComponent(this._getUrlParam(url, 'from'));
	        self.address_id = decodeURIComponent(this._getUrlParam(url, 'address_id'));
	        self.add = decodeURIComponent(this._getUrlParam(url, 'add'));
	        storage.getItem('carInfo', function (e) {
	            var result = e.result;
	            if (result === 'success') {
	                var data = e.data;
	                self.carInfo = JSON.parse(data);
	                self.lastAddrss.recipient_name = self.carInfo.car_name;
	                self.lastAddrss.recipient_phone = self.carInfo.login_phone;
	                self.name = self.carInfo.car_name;
	                self.phone = self.carInfo.login_phone;
	            } else {}
	        });
	        storage.getItem('userInfo', function (e) {
	            ...
	            if (result === 'success') {
	                ...
	                self.phone = self.userInfo.currentPhone;

	                if (self.add != 'add') {
	                    if (self.comeFrom && self.comeFrom.length > 0 && self.comeFrom == 'addressList') {
	        ...
	        storage.getItem('adv_activity_id', function (e) {
	            var result = e.result;
	            if (result === 'success') {
	                var data = e.data;
	                self.adv_activity_id = data;
	            } else {}
	        });
