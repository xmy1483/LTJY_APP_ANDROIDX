...
//    }
    public void handleDecode(final List<Result> resultList, Bundle bundle) {
        beepManager.playBeepSoundAndVibrate();
        //修改一月十五日

        if (resultList.size() == 2 && !resultList.get(0).getText().equals(resultList.get(1).getText())) {
           ...
           /* cameraManager.takePicture(new Camera.PictureCallback() {
                ...
                public void onPictureTaken(byte[] data, Camera camera) {
                    getData(data, resultList);
                }
            });*/
           cameraManager.getPreviewFrame(new Camera.PreviewCallback() {

               @Override
               public void onPreviewFrame(byte[] data, Camera camera) {
                   getData(data, resultList,camera);
               }
           });
        }
        else if (resultList.size()==1&&resultList.get(0).getText()!=null&&resultList.get(0).getText()!=""){
            // 不活动关闭 activity
            inactivityTimer.onActivity();
           /*
           Intent resultIntent = new Intent();
           ...
           /* cameraManager.takePicture(new Camera.PictureCallback() {
            ...
            // 扫描失败重新扫描
//            Toast.makeText(this, "请同时扫描两个二维码", Toast.LENGTH_SHORT).show();
            Toast.makeText(this, "扫描失败", Toast.LENGTH_SHORT).show();
            restartPreviewAfterDelay(1500);
        }
    }

    public abstract void getData(byte[] data, List<Result> resultList, Camera camera);
