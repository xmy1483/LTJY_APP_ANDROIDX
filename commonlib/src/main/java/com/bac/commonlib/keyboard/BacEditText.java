package com.bac.commonlib.keyboard;

import android.content.Context;
import android.content.res.TypedArray;
import android.graphics.Canvas;
import android.graphics.Color;
import android.graphics.Paint;
import android.graphics.RectF;
import androidx.annotation.Nullable;
import androidx.appcompat.widget.AppCompatTextView;
import android.text.InputFilter;
import android.util.AttributeSet;

import com.bac.commonlib.R;

import static android.graphics.Paint.ANTI_ALIAS_FLAG;


/**
 * Created by wujiazhen on 2017/7/7.
 */

public class BacEditText extends AppCompatTextView {

    private float cX;

    /**
     * view的高度
     */
    private int height;
    private int width;

    /**
     * 第一个圆开始绘制的圆心坐标
     */
    private float startX;
    private float startY;

    /**
     * 分割线开始的坐标x
     */
    private int divideLineWStartX;


    /**
     * 最大输入位数
     */
    private int maxCount = 6;
    /**
     * 圆的颜色   默认BLACK
     */
    private int circleColor = Color.BLACK;

    /**
     * 底部线的颜色   默认GRAY
     */
    private int bottomLineColor = Color.GRAY;

    /**
     * 实心圆的半径
     */
    private int radius = 10;

    /**
     * 分割线的宽度  默认2
     */
    private int divideLineWidth = 2;

    /**
     * 竖直分割线的颜色
     */
    private int divideLineColor = Color.GRAY;

    /**
     * 矩形边框的圆角
     */
    private int rectAngle = 0;

    /**
     * 是否明文，默认明文
     */
    private boolean psw = false;

    private int psdType = 0;

    private Context mContext;

    /**
     * 竖直分割线的画笔
     */
    private Paint divideLinePaint;
    /**
     * 圆的画笔
     */
    private Paint circlePaint;
    /**
     * 底部线的画笔
     */
    private Paint bottomLinePaint;
    /**
     * 分割线的画笔
     */
    private Paint borderPaint;
    /**
     * 分割线的颜色
     */
    private int borderColor = Color.GRAY;

    /**
     * 矩形框
     */
    private RectF rectF = new RectF();

    /**
     * 当前输入密码位数
     */
    private int textLength = 0;
    private int bottomLineLength;

    private final static int psdType_react = 0;
    private final static int psdType_bottomLine = 1;
    private Paint pText;
    private CharSequence text;
    private float textSize;

    public BacEditText(Context context) {
        this(context, null);
    }

    public BacEditText(Context context, @Nullable AttributeSet attrs) {
        this(context, attrs, 0);
    }

    public BacEditText(Context context, @Nullable AttributeSet attrs, int defStyleAttr) {
        super(context, attrs, defStyleAttr);

        mContext = context;

        getAtt(attrs);
        initPaint();

        this.setBackgroundColor(Color.TRANSPARENT);
        this.setCursorVisible(false);
        this.setFilters(new InputFilter[]{new InputFilter.LengthFilter(maxCount)});

    }

    private void getAtt(AttributeSet attrs) {

        TypedArray typedArray = mContext.obtainStyledAttributes(attrs, R.styleable.cl_bacEditText);
        maxCount = typedArray.getInt(R.styleable.cl_bacEditText_cl_maxCount, maxCount);
        circleColor = typedArray.getColor(R.styleable.cl_bacEditText_cl_circleColor, circleColor);
        bottomLineColor = typedArray.getColor(R.styleable.cl_bacEditText_cl_bottomLineColor, bottomLineColor);
        radius = typedArray.getDimensionPixelOffset(R.styleable.cl_bacEditText_cl_radius, radius);

        divideLineWidth = typedArray.getDimensionPixelSize(R.styleable.cl_bacEditText_cl_divideLineWidth, divideLineWidth);
        divideLineColor = typedArray.getColor(R.styleable.cl_bacEditText_cl_divideLineColor, divideLineColor);
        psdType = typedArray.getInt(R.styleable.cl_bacEditText_cl_psdType, psdType);
        rectAngle = typedArray.getDimensionPixelOffset(R.styleable.cl_bacEditText_cl_rectAngle, rectAngle);
        psw = typedArray.getBoolean(R.styleable.cl_bacEditText_cl_psw, psw);

        typedArray.recycle();

    }


    /**
     * 初始化画笔
     */
    private void initPaint() {

        pText = new Paint();
        pText.setColor(Color.BLACK);// 设置红色
        pText.setTextSize(100);
        pText.setAntiAlias(true);

        // startX , startY 居中的坐标
        textSize = pText.getTextSize() / 3;

        circlePaint = getPaint(5, Paint.Style.FILL, circleColor);

        bottomLinePaint = getPaint(2, Paint.Style.FILL, bottomLineColor);

        borderPaint = getPaint(3, Paint.Style.STROKE, borderColor);

        divideLinePaint = getPaint(divideLineWidth, Paint.Style.FILL, borderColor);

    }

    /**
     * 设置画笔
     *
     * @param strokeWidth 画笔宽度
     * @param style       画笔风格
     * @param color       画笔颜色
     * @return
     */
    private Paint getPaint(int strokeWidth, Paint.Style style, int color) {
        Paint paint = new Paint(ANTI_ALIAS_FLAG);
        paint.setStrokeWidth(strokeWidth);
        paint.setStyle(style);
        paint.setColor(color);
        paint.setAntiAlias(true);
        return paint;
    }

    @Override
    protected void onSizeChanged(int w, int h, int oldw, int oldh) {
        super.onSizeChanged(w, h, oldw, oldh);
        height = h;
        width = w;

        divideLineWStartX = w / maxCount;

        startX = w / maxCount / 2;
        startY = h / 2;

        bottomLineLength = w / (maxCount + 2);

        rectF.set(0, 0, width, height);

    }

    @Override
    protected void onDraw(Canvas canvas) {
        //不删除的画会默认绘制输入的文字,会在最顶部
        //super.onDraw(canvas);

        switch (psdType) {
            case psdType_react:
                drawWeChatBorder(canvas);
                break;
            case psdType_bottomLine:
                drawBottomBorder(canvas);
                break;
        }


        drawPsdCircle(canvas);
    }

    /**
     * 画微信支付密码的样式
     *
     * @param canvas
     */
    private void drawWeChatBorder(Canvas canvas) {

        canvas.drawRoundRect(rectF, rectAngle, rectAngle, borderPaint);

        for (int i = 0; i < maxCount - 1; i++) {
            canvas.drawLine((i + 1) * divideLineWStartX,
                    0,
                    (i + 1) * divideLineWStartX,
                    height,
                    divideLinePaint);
        }

    }


    /**
     * 画底部显示的分割线
     *
     * @param canvas
     */
    private void drawBottomBorder(Canvas canvas) {

        for (int i = 0; i < maxCount; i++) {
            cX = startX + i * 2 * startX;
            canvas.drawLine(cX - bottomLineLength / 2,
                    height,
                    cX + bottomLineLength / 2,
                    height, bottomLinePaint);
        }
    }

    /**
     * 画密码实心圆
     *
     * @param canvas
     */
    private void drawPsdCircle(Canvas canvas) {
        for (int i = 0; i < textLength; i++) {

            if (psw) {
                canvas.drawCircle(
                        startX + i * 2 * startX,// 其实位置
                        startY, // 容器高度的一般
                        radius,
                        circlePaint);
            }else{
                canvas.drawText(text.charAt(i)+"",  startX + i * 2 * startX- textSize, startY + textSize, pText);// 画文本
            }
        }


    }


    @Override
    protected void onTextChanged(CharSequence text, int start, int lengthBefore, int lengthAfter) {
        super.onTextChanged(text, start, lengthBefore, lengthAfter);
        this.text = text;
        // 文字变换
        textLength = text.toString().length();

        invalidate();

    }


}
